...
""" #Setup plages horaires (dépendant du cursus suivi/bâtiment). Ici: ANTHROPOLE
...
class UnilScrape:
    JOURS = ["Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi"]
    def __init__(self, database, faculte):
    ...
    def scrape(self, url, plages=None, exclude=None):
        if plages is None:
            plages = ["08:30", "09:15", "10:15", "11:15", "12:30", "13:15", "14:15", "15:15", "16:15", "17:15"]
        if exclude is None:
    ...
    def cours_to_database(self, cours_dict):
        #get id of branche
        ...
        for horaire in cours_dict['horaires']:
            for class_begin in horaire[1]:
                ...
                self.cur.execute("INSERT INTO cours_has_horaire (coursid, horaireid, brancheid) VALUES (?, ?, ?)",
                            (coursid, id_horaire, brancheid)
                            )
    
    def branches_to_database(self, branche):
        #insert data branche
        self.cur.execute("INSERT INTO branche (nom, faculte) VALUES (?, ?)",
                    (branche, self.faculte)
                    )

    def create_horaires(self, plages):
